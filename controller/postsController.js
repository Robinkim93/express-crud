...
const postsModify = async (req, res, next) => {
  const { userId, postingId } = req.query;
  const { content } = req.body;
  if (!userId || !postingId || !content) {
    return res.status(400).json({ message: "KEY_ERROR" });
  }
  const data = await postsService.postsModify(userId, postingId, content);
  return res.status(200).json({ message: data });
};
